...
- **Primary product goal**: Provide a fast, offline-capable mobile experience for exploring ATIO KB innovations (browse, filter, drill-down, view details).
...
- **Design choice today**: Package the knowledge base as a **local SQLite database** bundled with the app, so browsing is instant and works offline.
...
1. **Mobile app (Expo React Native)** — offline-first browsing + UI.
...
- **Search**: `POST /api/search`
  ...
  - Stage 2: optional LLM rerank (OpenAI) using *sanitized* long-text only
...
- However, **core KB browsing does not use the backend** today; it reads SQLite directly for speed/offline guarantees.

---

## Data flow (today)

### “Explore / Browse” path (offline-first)
...
3. UI renders immediately from local queries; no network required.
...
4. App renders results; if the backend is unreachable, the UI shows an error and continues to support offline browsing.

### “AI bullets in detail drawer” (online with local cache)

1. When the user opens a solution’s drawer, the client sends **only the description text** to `POST /api/summarize-bullets`.
2. The result is cached in the local SQLite table `innovation_bullet_cache` keyed by `innovation_id`.
3. Subsequent opens for the same innovation use the cache and avoid repeated AI calls.
...
## Synchronization strategy (future)

In the future, you want the app to remain synchronized with the ATIO KB “over time” while preserving the offline-first UX. The recommended model is:
...
### Key design principle: offline-first + server-driven freshness
...
- The existing backend endpoints are unauthenticated (dev-friendly). For production sync you will likely need:
  ...
  - request throttling
  - careful PII/content controls (especially around what text is sent to LLMs)
...
- Today, ATIO KB Mobile is **offline-first**: the KB content is a bundled SQLite snapshot (`assets/db/atiokb.db`) accessed directly by the app.
...
- The recommended future integration is a secure **KB Sync API** that supports versioned snapshots first (fastest path), then deltas/streaming for near-real-time updates—while keeping local SQLite as the on-device cache for speed and offline use.
...
**Why this approach?** The ATIO website is behind authentication and its API cannot be opened to the public. Preparing JSON and uploading it to SQLite provides a controlled, offline-capable snapshot that can be distributed with the app without exposing credentials or requiring network access for core content.
